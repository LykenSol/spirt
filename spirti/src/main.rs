...
use spirt::{
    AddrSpace, Attr, Const, ConstKind, Context, DataInst, DataInstDef, DataInstKind, DeclDef,
    EntityOrientedDenseMap, ExportKey, Exportee, Func, GlobalVar, InternedStr, Module, Node,
    NodeDef, NodeKind, Region, RegionDef, SelectionKind, Type, TypeKind, Value, Var, scalar, spv,
    vector,
};
use std::ops::Range;
use std::path::Path;
...
impl<'a> Interpreter<'a> {
    ...
    fn eval_region(
        &mut self,
        func_at_region: FuncAt<Region>,
        input_vals: SmallVec<[DynVal; 4]>,
    ) -> SmallVec<[DynVal; 4]> {
        let RegionDef { inputs, children: _, outputs } = func_at_region.def();

        for (&input_var, input) in inputs.iter().zip_eq(input_vals) {
    ...
    fn eval_node(&mut self, func_at_node: FuncAt<Node>) {
        let NodeDef { attrs: _, inputs, kind, child_regions, outputs } = func_at_node.def();
        match kind {
            NodeKind::Select(kind) => {
                let case_idx = match kind {
                    SelectionKind::BoolCond => {
                        self.inst_counter += 1;

                        let cond = match self.eval_value(inputs[0]) {
                        ...
                        let cond = match cond {
                            ...
                            DynLeaf::PerInvocation(_) => {
                                todo!("per-invocation `if`-`else` decision")
                            }
                        };
                        if cond { 0 } else { 1 }
                    }
                ...
                let case = child_regions[case_idx];

                let output_vals = self.eval_region(func_at_node.at(case), SmallVec::new());

                for (&output_var, output) in outputs.iter().zip_eq(output_vals) {
                    self.call_stack.last_mut().unwrap().var_values.insert(output_var, output);
                }
            }
            NodeKind::Loop { repeat_condition } => {
                let mut loop_state = inputs.iter().map(|&v| self.eval_value(v)).collect();

                loop {
                    loop_state = self.eval_region(func_at_node.at(child_regions[0]), loop_state);
            ...
            // FIXME(eddyb) merge these post-`DataInst`-refactor.
            DataInstKind::Scalar(_)
            | DataInstKind::Vector(_)
            | DataInstKind::FuncCall(_)
            | DataInstKind::QPtr(_)
            | DataInstKind::SpvInst(_)
            | DataInstKind::SpvExtInst { .. } => self.eval_data_inst(func_at_node),
        }
    }
    fn eval_data_inst(&mut self, func_at_inst: FuncAt<DataInst>) {
        ...
        let DataInstDef { attrs: _, kind, inputs, child_regions: _, outputs } = func_at_inst.def();

        let inputs: SmallVec<[_; 4]> = inputs.iter().map(|&v| self.eval_value(v)).collect();
        let output_type = outputs.first().map(|&var| func_at_inst.vars[var].ty);
        ...
        if let Some(output) = output {
            self.call_stack.last_mut().unwrap().var_values.insert(outputs[0], output);
        }
        let elapsed = start.elapsed();
