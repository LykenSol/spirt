...
use spirt::{
    ...
    NodeDef, NodeKind, Region, RegionDef, SelectionKind, Type, TypeKind, Value, scalar, spv,
    vector,
};
use std::ops::Range;
use std::path::Path;
...
#[derive(Default)]
struct CallFrame {
    region_inputs: EntityOrientedDenseMap<Region, SmallVec<[DynVal; 4]>>,
    node_outputs: EntityOrientedDenseMap<Node, SmallVec<[DynVal; 4]>>,
...
impl<'a> Interpreter<'a> {
    ...
    fn eval_call(&mut self, f: Func, args: SmallVec<[DynVal; 4]>) -> SmallVec<[DynVal; 4]> {
        let func_def_body = match &self.module.funcs[f].def {
            DeclDef::Imported(import) => unreachable!("calling import {:?}", match import {
    ...
    fn eval_node(&mut self, func_at_node: FuncAt<Node>) {
        let NodeDef { attrs: _, inputs, kind, outputs } = func_at_node.def();
        match kind {
            ...
            NodeKind::Select { kind, cases } => match kind {
                SelectionKind::BoolCond => {
                    ...
                    if !output_vals.is_empty() {
                        self.call_stack
                            ...
                            .unwrap()
                            .node_outputs
                            .insert(func_at_node.position, output_vals);
                    }
                }
            ...
            NodeKind::Loop { body, repeat_condition } => {
                ...
                loop {
                    loop_state = self.eval_region(func_at_node.at(*body), loop_state);
    ...
    fn eval_data_inst(&mut self, func_at_inst: FuncAt<DataInst>) {
        ...
        let DataInstDef { attrs: _, kind, inputs, output_type } = func_at_inst.def();
    ...
    fn eval_value(&mut self, value: Value) -> DynVal {
        match value {
            ...
            Value::NodeOutput { node, output_idx } => {
                self.call_stack.last().unwrap().node_outputs[node][output_idx as usize].clone()
            }
