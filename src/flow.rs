...
use crate::{
    AttrSet, ConstDef, ConstKind, Context, DataInst, DataInstDef, DataInstKind, Diag, FuncDefBody,
    FxIndexMap, FxIndexSet, Node, NodeDef, NodeKind, NodeOutputDecl, Region, RegionDef,
    RegionInputDecl, Type, Value, cfg,
};
...
enum ObjectStateDef {
    AfterInst {
        ...
        inst: DataInst,
        // HACK(eddyb) parent region of `inst`, only needed to be able to delete it.
...
pub fn flow_func(cx: Rc<Context>, func_def_body: &mut FuncDefBody) {
    ...
    let (objects, object_states, loop_object_state_initial_and_after_body, replaceable_insts) = {
        ...
        flow_cx.flow_through_region(func_def_body.at_mut_body());
        (
            flow_cx.objects,
    ...
    let mut remove_inst = |inst, parent_region| {
        func_def_body.regions[parent_region].children.remove(inst, &mut func_def_body.nodes)
    };
    for (original_inst, (_, parent_region)) in replaceable_insts {
        remove_inst(original_inst, parent_region);
    }
    ...
    while let Some(state) = state_visiting_queue.pop_front() {
        ...
        match &object_states[state.0] {
            &ObjectStateDef::AfterInst { prev, inst, parent_region } => {
                remove_inst(inst, parent_region);
...
struct FlowContext<'a> {
    ...
    // nodes at a cost proportional only to the number of changes
    ...
    replaceable_insts: FxIndexMap<DataInst, (SmallVec<[Option<Value>; 2]>, Region)>,
}
...
impl FlowContext<'_> {
    ...
    fn query_past_value_inner(
        ...
        match &self.object_state_interner[current.0] {
            ...
            ObjectStateDef::AfterSelect { select_node, state_before_select, per_case_states } => {
                ...
                select_output_decls
                    .push(NodeOutputDecl { attrs: Default::default(), ty: value_type });

                for (case_idx, per_case_value) in per_case_values.into_iter().enumerate() {
                    let case = func.reborrow().at(select_node).def().child_regions[case_idx];
                ...
                Ok(Value::NodeOutput { node: select_node, output_idx })
            }
            &ObjectStateDef::BeforeLoopBody { loop_node, .. } => {
                ...
                let loop_node_def = func.reborrow().at(loop_node).def();
                let initial_inputs = &mut loop_node_def.inputs;
                let body = loop_node_def.child_regions[0];
                ...
                body_input_decls
                    .push(RegionInputDecl { attrs: Default::default(), ty: value_type });

                let new_body_input = Value::RegionInput { region: body, input_idx };
    ...
    fn flow_into_values(&mut self, values: &mut [Value]) {
        for v in values {
            ...
            if let Value::NodeOutput { node: inst, output_idx } = *v {
    ...
    fn flow_through_data_inst(
        ...
        // FIXME(eddyb) the qptr pnp code used `self.parent_region` instead.
        parent_region: Region,
    ) {
        let cx = self.cx;
        ...
        let DataInstDef { attrs: _, kind, inputs, child_regions: _, outputs } =
            func_at_inst.reborrow().def();

        // HACK(eddyb) already handled by `flow_through_node`.
        ...
        match *kind {
            ...
            DataInstKind::QPtr(QPtrOp::FuncLocalVar(_)) => {
                ...
                self.value_to_capability
                    .insert(Value::NodeOutput { node: data_inst, output_idx: 0 }, new_cap);
            }
                ...
                if let Some(known_state) = known_state {
                    ...
                    let loaded_value = self.query_past_value(
                        ...
                        |func_at_write_inst| {
                            ...
                            match (&write_inst_def.kind, &write_inst_def.inputs[..]) {
                                (DataInstKind::QPtr(QPtrOp::FuncLocalVar(_)), []) => {
                    ...
                    if let Ok(loaded_value) = loaded_value {
                        self.replaceable_insts.insert(
                            data_inst,
                            ([Some(loaded_value)].into_iter().collect(), parent_region),
                        );
                        return;
                    }
                }
    ...
    fn flow_through_region(&mut self, mut func_at_region: FuncAtMut<'_, Region>) {
        ...
        while let Some(func_at_node) = children.next() {
            self.flow_through_node(func_at_node, region);
        }
    ...
    fn flow_through_node(
        ...
        self.flow_into_values(&mut node_def.inputs);
        match &mut node_def.kind {
            NodeKind::Select(_) => {
                // FIXME(eddyb) deduplicate after inlining `flow_through_data_inst`
                // (also see how pnp looks now).
                self.unanalyzable_value_uses(&node_def.inputs);
                ...
                if num_cases <= 1 {
                    if num_cases == 1 {
                        let case = node_def.child_regions[0];
                        self.flow_through_region(func.at(case));
                    }
                    return;
                }
                ...
                for case_idx in 0..num_cases {
                    ...
                    self.flow_through_region(func.reborrow().at(case));
            ...
            NodeKind::Loop { repeat_condition: _ } => {
                        ...
                        .map(|obj| {
                            (obj, LoopObjectState {
                                ...
                                before_body: self.intern_object_state(
                                    ObjectStateDef::BeforeLoopBody { loop_node: node, object: obj },
                                ),
                                after_body: None,
                            })
                        })
                ...
                loop {
                    ...
                    self.flow_through_region(func.reborrow().at(body));

                    // NOTE(eddyb) the repeat condition is technically part of
                    // the loop body, as if it were an extra region output.
                    let repeat_condition = match &mut func.reborrow().at(node).def().kind {
                        NodeKind::Loop { repeat_condition, .. } => repeat_condition,
                        _ => unreachable!(),
                    };
            ...
            | DataInstKind::SpvExtInst { .. } => {
                self.flow_through_data_inst(func.at(node), parent_region);
            }
        }
    }
}
