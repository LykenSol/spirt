...
impl<'a> GatherAccesses<'a> {
    ...
    fn gather_accesses_in_func_uncached(
        ...
        for &node in post_order_nodes.iter().rev() {
            let mut per_output_accesses =
                node_to_per_output_accesses.remove(&node).unwrap_or_default();
                    ...
                    Value::Var(ptr) => match func_def_body.at(ptr).decl().kind() {
                        ...
                        VarKind::RegionInput { .. } => {
                            accesses_or_err_attrs_to_attach.extend([
                                (AttrTarget::Var(ptr), new_accesses),
                                (
                                    AttrTarget::Var(ptr),
                                    Err(AnalysisError(Diag::bug([
        ...
        if !node_to_per_output_accesses.is_empty() {
            let mut attach_as_err = |v, accesses: Option<Result<_, _>>| {
                let diag = match accesses.transpose() {
                    Ok(accesses) => Diag::bug([
                        "extra mem.accesses contributions ignored (visited too late): ".into(),
                        accesses.unwrap_or(MemAccesses::Data(DataHapp::DEAD)).into(),
                    ]),
                    Err(AnalysisError(mut diag)) => {
                        diag.message.insert(
                            0,
                            "extra mem.accesses-related errors ignored (visited too late): ".into(),
                        );
                        diag
                    }
                };
                accesses_or_err_attrs_to_attach.push((v, Err(AnalysisError(diag))));
            };

            for &node in &post_order_nodes {
                let per_output_accesses =
                    node_to_per_output_accesses.remove(&node).unwrap_or_default();

                if per_output_accesses.is_empty() {
                    continue;
                }

                let node_def = func_def_body.at(node).def();
                for (&output_var, accesses) in node_def.outputs.iter().zip(per_output_accesses) {
                    attach_as_err(AttrTarget::Var(output_var), accesses);
                }
            }
            ...
            assert_eq!(node_to_per_output_accesses.len(), 0);
        }
