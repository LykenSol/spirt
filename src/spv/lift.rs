...
impl<AI: AllocIds> Visitor<'_> for Lifter<'_, AI> {
    ...
    fn visit_data_inst_form_def(&mut self, data_inst_form_def: &DataInstFormDef) {
        match data_inst_form_def.kind {
            ...
            DataInstKind::Scalar(_) | DataInstKind::Vector(_) | DataInstKind::SpvInst(..) => {}
...
struct FuncBodyLifting<'a> {
    ...
    data_insts: EntityOrientedDenseMap<DataInst, DataInstOrFuncCallLifting>,
    calls: FxHashMap<ControlNode, DataInstOrFuncCallLifting>,
...
struct DataInstOrFuncCallLifting {
...
impl FuncAt<'_, ControlRegion> {
    ...
    fn rev_post_order_for_each_inner(
        ...
        for func_at_control_node in self.at_children() {
            func_at_control_node.rev_post_order_for_each_inner(f, &CfgCursor {
    ...
    fn from_func_def_body(
        ...
        let mut visit_cfg_point = |point_cursor: CfgCursor<'_>| {
            ...
            let insts = match point {
                CfgPoint::ControlNodeEntry(control_node) => {
                    let func_at_control_node = func_def_body.at(control_node);
                    match func_at_control_node.def().kind {
                        ControlNodeKind::Block { insts } => {
                            for func_at_inst in func_def_body.at(insts) {
                                data_insts.insert(
                                    func_at_inst.position,
                                    DataInstOrFuncCallLifting::from_inst_or_call(
                                        lifter,
                                        Either::Left(func_at_inst),
                                    ),
                                );
                            }
                        ...
                        ControlNodeKind::FuncCall { .. } => {
                            calls.insert(
                                ...
                                DataInstOrFuncCallLifting::from_inst_or_call(
                                    lifter,
                                    Either::Right(func_at_control_node),
                                ),
                            );
...
impl DataInstOrFuncCallLifting {
    fn from_inst_or_call(
        lifter: &mut Lifter<'_, impl AllocIds>,
        func_at_inst_or_call: Either<FuncAt<'_, DataInst>, FuncAt<'_, ControlNode>>,
    ) -> Self {
        let wk = &spec::Spec::get().well_known;
        let cx = lifter.cx;

        let def = func_at_inst_or_call.map_either(
            |func_at_inst| &cx[func_at_inst.def().form],
            |func_at_call| func_at_call.def(),
        );
        let kind = def.map_either(|def| &def.kind, |def| &def.kind);
        let output_types = def.map_either(
            |inst_form_def| inst_form_def.output_types.iter().copied(),
            |call_def| call_def.outputs.iter().map(|o| o.ty),
        );

        let mut new_spv_inst_lowering = spv::InstLowering::default();
        let spv_inst_lowering = match kind {
            // Disallowed while visiting.
            Either::Left(DataInstKind::QPtr(_)) => unreachable!(),

            Either::Left(DataInstKind::Scalar(_) | DataInstKind::Vector(_)) => {
            ...
            Either::Right(ControlNodeKind::FuncCall { callee, .. }) => {
                if output_types.len() > 1 {
        ...
        let has_result = matches!(kind, Either::Right(ControlNodeKind::FuncCall { .. }))
        ...
        DataInstOrFuncCallLifting { result_id, disaggregate_result, reaggregate_inputs }
    }
...
enum LazyInst<'a, 'b> {
    ...
    DataInstOrFuncCall {
        parent_func_ids: &'b FuncIds<'a>,
        def: Either<&'a DataInstDef, &'a ControlNodeDef>,
        lifting: &'b DataInstOrFuncCallLifting,
    },
...
impl LazyInst<'_, '_> {
    fn result_id_attrs_and_import(
        ...
        match self {
            ...
            Self::DataInstOrFuncCall { parent_func_ids: _, def, lifting } => {
                (lifting.result_id, def.either(|def| def.attrs, |def| def.attrs), None)
            }
    ...
    fn for_each_spv_inst_with_debug_line(
        ...
        let value_to_id = |parent_func_ids: &FuncIds<'_>, v| match v {
            ...
            Value::ControlNodeOutput { control_node, output_idx } => {
                let parent_func_body_lifting = parent_func_ids.body.as_ref().unwrap();
                match parent_func_body_lifting.calls.get(&control_node) {
                    Some(call_lifting) => call_lifting.id_for_output(output_idx),
                    None => {
                        parent_func_body_lifting.blocks[&CfgPoint::ControlNodeExit(control_node)]
            ...
            Value::DataInstOutput { inst, output_idx } => {
                parent_func_ids.body.as_ref().unwrap().data_insts[inst].id_for_output(output_idx)
            }
        };
        ...
        match self {
            ...
            Self::DataInstOrFuncCall { parent_func_ids, def, lifting } => {
                let kind = def.map_either(|def| &cx[def.form].kind, |def| &def.kind);
                let inputs = def.either(
                    |inst_def| &inst_def.inputs,
                    |call_def| match &call_def.kind {
                        ControlNodeKind::FuncCall { inputs, .. } => inputs,
                        _ => unreachable!(),
                    },
                );
                let output_types = def.map_either(
                    |inst_def| cx[inst_def.form].output_types.iter().copied(),
                    |call_def| call_def.outputs.iter().map(|o| o.ty),
                );
                ...
                let (inst, spv_inst_lowering) = match kind
                    .map_left(|kind| spv::Inst::from_canonical_data_inst_kind(kind).ok_or(kind))
                {
                    Either::Left(Ok(spv_inst)) => {
                        // FIXME(eddyb) deduplicate creating this `OpTypeStruct`.
                        if output_types.len() > 1 {
                    ...
                    Either::Left(Err(DataInstKind::Scalar(_) | DataInstKind::Vector(_))) => {
                        unreachable!("should've been handled as canonical")
                    }

                    // Disallowed while visiting.
                    Either::Left(Err(DataInstKind::QPtr(_))) => unreachable!(),

                    Either::Left(Err(DataInstKind::SpvInst(inst, lowering))) => {
                        (inst.clone(), lowering)
                    }
                    Either::Left(Err(DataInstKind::SpvExtInst { ext_set, inst, lowering })) => {
                ...
                for id_operand in spv_inst_lowering.reaggreate_inputs(inputs) {
            ...
            ids.funcs.iter().flat_map(|(&func, func_ids)| {
                ...
                let body_insts = body_with_lifting.map(|(func_def_body, func_body_lifting)| {
                    func_body_lifting.blocks.iter().flat_map(move |(point, block)| {
                        ...
                        .chain(insts.iter().copied().flat_map(move |insts| {
                            match insts {
                                BlockInsts::DataInsts(insts) => {
                                    Either::Left(func_def_body.at(insts).into_iter().map(
                                        move |func_at_inst| LazyInst::DataInstOrFuncCall {
                                            parent_func_ids: func_ids,
                                            def: Either::Left(func_at_inst.def()),
                                            lifting: &func_body_lifting.data_insts
                                                [func_at_inst.position],
                                        },
                                    ))
                                }
                                BlockInsts::FuncCall(call_node) => Either::Right(
                                    [LazyInst::DataInstOrFuncCall {
                                        parent_func_ids: func_ids,
                                        def: Either::Right(func_def_body.at(call_node).def()),
                                        lifting: &func_body_lifting.calls[&call_node],
                                    }]
                                    .into_iter(),
                                ),
                            }
