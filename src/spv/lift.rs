...
use crate::{
    ...
    ExportKey, Exportee, Func, FuncDecl, FuncDefBody, FuncParam, FxIndexMap, FxIndexSet, GlobalVar,
                    ...
                    match control_node_def.kind {
                            ...
                            for func_at_inst in func_def_body.at(insts) {
                                if cx[func_at_inst.def().form].output_type.is_some() {
                                    data_inst_output_ids
                                        .insert(func_at_inst.position, (id_allocator.alloc_id)());
                                }
                            }
                        ...
                        ControlNodeKind::FuncCall { .. } => {
                            ...
                            if !control_node_def.outputs.is_empty() {
                                call_output_ids.insert(control_node, (id_allocator.alloc_id)());
                            }
...
impl LazyInst<'_, '_> {
    fn result_id_attrs_and_import(
        ...
        match self {
            ...
            Self::FuncCall { parent_func_ids: _, result_id, control_node_def } => {
    ...
    fn to_inst_and_attrs(
        ...
        let value_to_id = |parent_func_ids: &FuncIds<'_>, v| match v {
            ...
            Value::ControlNodeOutput { control_node, output_idx } => {
                let parent_func_body_lifting = parent_func_ids.body.as_ref().unwrap();
                match parent_func_body_lifting.call_output_ids.get(&control_node) {
                    Some(&call_result_id) => {
        ...
        let inst = match self {
            ...
            Self::FuncCall { parent_func_ids, result_id: _, control_node_def } => {
                assert!(control_node_def.outputs.len() <= 1);
                ...
                spv::InstWithIds {
                    ...
                    ids: [ids.funcs[callee].func_id]
                        .into_iter()
                        .chain(inputs.iter().map(|&v| value_to_id(parent_func_ids, v)))
                        .collect(),
                }
            }
                ...
                let body_insts = body_with_lifting.map(|(func_def_body, func_body_lifting)| {
                    func_body_lifting.blocks.iter().flat_map(move |(point, block)| {
                        ...
                        .chain(insts.iter().copied().flat_map(move |insts| match insts {
                            BlockInsts::DataInsts(insts) => Either::Left(
                                func_def_body.at(insts).into_iter().map(move |func_at_inst| {
                            ...
                            BlockInsts::FuncCall(call_node) => {
                                let control_node_def = func_def_body.at(call_node).def();
                                Either::Right(
                                    [LazyInst::FuncCall {
                                        parent_func_ids: func_ids,
                                        result_id: (!control_node_def.outputs.is_empty())
                                            .then(|| func_body_lifting.call_output_ids[&call_node]),
                                        control_node_def,
                                    }]
                                    .into_iter(),
                                )
