...
use crate::{
    ...
    Import, Module, ModuleDebugInfo, ModuleDialect, Node, NodeKind, OrdAssertEq, Region, Type,
    TypeDef, TypeKind, TypeOrConst, Value, Var, VarDecl, VarKind, scalar,
};
                    ...
                    if let Some(terminator) = unstructured_terminator {
                        ...
                        Terminator {
                            ...
                            reaggregated_return_value_id: match kind {
                                cf::unstructured::ControlInstKind::Return if inputs.len() > 1 => {
        ...
        match self {
            ...
            Self::DataInst { parent_func_ids, data_inst_def, lifting } => {
                    ...
                    .ok_or(kind)
                {
                    ...
                    Err(DataInstKind::Mem(_) | DataInstKind::QPtr(_)) => {
                        // Disallowed while visiting.
                        unreachable!()
                    }
            ...
            Self::Terminator { parent_func_ids, terminator } => {
                ...
                if let Some(reaggregated_value_id) = terminator.reaggregated_return_value_id {
                    assert!(
                        matches!(*terminator.kind, cf::unstructured::ControlInstKind::Return)
                ...
                let inst = match &*terminator.kind {
                    ...
                    cf::unstructured::ControlInstKind::SelectBranch(SelectionKind::Switch {
                        case_consts,
                    }) => {
