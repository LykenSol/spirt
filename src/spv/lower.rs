...
use crate::{
    ...
    NodeKind, Region, RegionDef, SelectionKind, Type, TypeDef, TypeKind, TypeOrConst, Value, Var,
    VarDecl, cfg, print, scalar,
};
        ...
        for func_body in pending_func_bodies {
            ...
            impl VarRange {
                ...
                fn iter<'a>(
                    &self,
                    func: FuncAt<'a, ()>,
                ) -> impl ExactSizeIterator<Item = Value> + 'a {
            ...
            for (raw_inst_idx, raw_inst) in raw_insts.iter().enumerate() {
                ...
                impl<'a> Leaves<'a> {
                    fn iter<'b>(
                        &self,
                        func: FuncAt<'b, ()>,
                    ) -> impl ExactSizeIterator<Item = Value> + 'b
                    where
                        'a: 'b,
                    {
                        match *self {
                ...
                if opcode == wk.OpPhi {
                    ...
                    local_id_defs.insert(result_id.unwrap(), LocalIdDef::Value {
                        whole_type: result_type,
                        leaves: NonZeroU32::new(end - start)
                ...
                let eagerly_lowered_result = if opcode == wk.OpCompositeConstruct {
                    ...
                    (|| {
                        ...
                        let component_leaves = if !non_aggregate_indexing_imms.is_empty() {
                            ...
                            let leaf_inputs = replace_component
                                ...
                                .collect();
                            let leaf_inst = append_node(func_def_body.at_mut(()), DataInstDef {
                                attrs,
                                kind: leaf_spv_inst
                                    .as_canonical_data_inst_kind(
                                        &cx,
                            ...
                            let leaf_output_var = func_def_body.vars.define(&cx, VarDecl {
                                // FIXME(eddyb) this may lose semantic `attrs`.
                                attrs: AttrSet::default(),
                                ty: leaf_output_type,
                ...
                let node = append_node(func_def_body.at_mut(()), NodeDef {
                    attrs,
                    kind,
                    inputs,
                    child_regions: [].into_iter().collect(),
                ...
                // node, just to keep the "output type iterator" part simpler.
                ...
                if let DataInstKind::SpvInst(spv_inst, lowering) = &node_def.kind {
                    if lowering.disaggregated_inputs.is_empty() {
                        if let Some(canonical_kind) = spv_inst.as_canonical_data_inst_kind(
                            &cx,
                            node_def
                                .outputs
                                .iter()
