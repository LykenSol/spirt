...
use crate::{
    ...
    GlobalVarInit, Import, InternedStr, Module, SelectionKind, Type, TypeDef, TypeKind,
    TypeOrConst, Value, cfg, print, scalar,
};
    ...
    pub fn lower_from_spv_module_parser(
        ...
        for func_body in pending_func_bodies {
            ...
            impl ValueRange {
                fn iter(&self) -> impl ExactSizeIterator<Item = Value> + Clone {
                    ...
                    (0..self.count).map(move |i| match start {
                        ...
                        Value::DataInstOutput { inst, output_idx } => Value::DataInstOutput {
                            inst,
                            output_idx: output_idx.checked_add(i).unwrap(),
                        },
                    })
            ...
            for (raw_inst_idx, raw_inst) in raw_insts.iter().enumerate() {
                    ...
                    if let Some(func_def_body) = &mut func_def_body {
                        ...
                        local_id_defs.insert(result_id.unwrap(), LocalIdDef::Value {
                            ...
                            leaves: Either::Left(ValueRange {
                                start: Value::ControlRegionInput {
                    ...
                    current_block.shadowed_local_id_defs.extend(
                        current_block.details.cfgssa_inter_block_uses.iter().map(
                            |(&used_id, &ty)| {
                                ...
                                inputs.extend(ty.disaggregated_leaf_types(&cx).map(|ty| {
                                    ControlRegionInputDecl { attrs: AttrSet::default(), ty }
                                }));
                ...
                let mut append_inst_or_node = |def: Either<DataInstDef, ControlNodeDef>| {
                    let data_inst = match def {
                        ...
                        Either::Right(control_node_def) => {
                            ...
                            return Either::Right(control_node);
                        }
                    };
                ...
                let eagerly_lowered_result = if opcode == wk.OpCompositeConstruct {
                    ...
                    (|| {
                        ...
                        let component_leaves = if !non_aggregate_indexing_imms.is_empty() {
                            ...
                            let leaf_inst = append_inst_or_node(Either::Left(DataInstDef {
                                attrs,
                ...
                let mut kind = if opcode == wk.OpFunctionCall {
                    ...
                    match maybe_callee {
                        ...
                        None => Either::Left(DataInstKind::SpvInst(
                            raw_inst.without_ids.clone(),
                            spv::InstLowering::default(),
                        )),
                    }
                    ...
                    Either::Left(DataInstKind::SpvExtInst {
                        ext_set,
                        inst,
                        lowering: spv::InstLowering::default(),
                    })
                } else {
                    Either::Left(DataInstKind::SpvInst(
                        raw_inst.without_ids.clone(),
                        spv::InstLowering::default(),
                    ))
                };
                ...
                let call_ret_type = match kind {
                    Either::Right(ControlNodeKind::FuncCall { .. }) => Some(result_type.unwrap()),
                    _ => None,
                };
                ...
                let mut spv_inst_lowering = match &mut kind {
                    ...
                    Either::Right(ControlNodeKind::FuncCall { .. }) => None,
                ...
                if let Either::Left(DataInstKind::SpvInst(spv_inst, lowering)) = &kind {
                    if lowering.disaggregated_inputs.is_empty() {
                            ...
                            spv_inst.as_canonical_data_inst_kind(&cx, &output_types)
                        {
                            ...
                            kind = Either::Left(canonical_kind);
                        }
                    }
                }

                let inst_or_node = append_inst_or_node(kind.map_either_with(
                    (inputs, output_types),
                    |(inputs, output_types), kind| DataInstDef {
                        attrs,
                        form: cx.intern(DataInstFormDef { kind, output_types }),
                        inputs,
                    },
                    |(inputs, output_types), mut kind| {
                        assert!(
                            mem::replace(
                                match &mut kind {
                ...
                if let Some(result_id) = result_id {
                    local_id_defs.insert(result_id, LocalIdDef::Value {
                        ...
                        leaves: Either::Left(ValueRange {
                            start: inst_or_node.either(
                                |inst| Value::DataInstOutput { inst, output_idx: 0 },
                                |control_node| Value::ControlNodeOutput {
                                    control_node,
                                    output_idx: 0,
                                },
                            ),
                            count: u32::try_from(output_types_len).unwrap(),
                        }),
                    });
                }
            }
