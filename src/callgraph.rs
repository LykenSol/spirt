...
use crate::{
    ...
    Node, NodeDef, NodeKind, NodeOutputDecl, Region, RegionDef, RegionInputDecl, SelectionKind,
    Type, TypeKind, Value, scalar,
};
...
impl Visitor<'_> for CallGraphCollector<'_> {
    ...
    // handles whole `Region`s instead of just their `Node`s.
    fn visit_region_def(&mut self, func_at_region: FuncAt<'_, Region>) {
        let RegionDef { inputs, children, outputs } = func_at_region.def();

        for input in inputs {
            input.inner_visit_with(self);
        }
        for func_at_node in func_at_region.at(*children) {
            let NodeDef { attrs, kind, inputs, child_regions: _, outputs } = func_at_node.def();
            if let (Ok(caller), NodeKind::FuncCall { callee }) = (self.caller, kind) {
    ...
    fn visit_node_def(&mut self, _: FuncAt<'_, Node>) {
        unreachable!()
    }
}
        ...
        for (func_idx, (&func, func_states)) in per_func_states.iter().enumerate() {
            ...
            let current_state = {
                func.reborrow()
                    ...
                    .def()
                    .inputs
                    .push(RegionInputDecl { attrs: AttrSet::default(), ty: state_ty });
                Value::RegionInput { region: new_body, input_idx: 0 }
            };
                ...
                let next_state_after =
                    match per_func_states.get_index(func_idx + 1).map(|(&f, _)| f) {
                        Some(next_func_in_group) => {
                            ...
                            let call_node = func.nodes.define(
                                cx,
                                NodeDef {
                                    attrs: AttrSet::default(),
                                    kind: NodeKind::FuncCall { callee: next_func_in_group },
                            ...
                            children.insert_last(call_node, func.nodes);
                            Value::NodeOutput { node: call_node, output_idx: 0 }
                        }
            ...
            let state_switch_node = {
                ...
                func.nodes.define(
                    ...
                    NodeDef {
                        ...
                        inputs: [current_state].into_iter().collect(),
                        child_regions: cases,
                        outputs: [NodeOutputDecl { attrs: AttrSet::default(), ty: state_ty }]
                            .into_iter()
                            .collect(),
                    }
                    .into(),
            ...
            new_body_def.outputs.push(Value::NodeOutput { node: state_switch_node, output_idx: 0 });
        }
    }
    ...
    fn transform_inter_emu_group_func_call(
        ...
        let call_node_outputs = {
            ...
            (0..u32::try_from(count).unwrap())
                .map(|output_idx| Value::NodeOutput { node: call_site.func_call_node, output_idx })
        };
        ...
        let current_state = {
            func.reborrow()
                ...
                .inputs
                .push(RegionInputDecl { attrs: AttrSet::default(), ty: state_ty });
            Value::RegionInput { region: state_machine_loop_body, input_idx: 0 }
        };
        let next_state = {
            let call_node = func.nodes.define(
                cx,
                NodeDef {
                    attrs: AttrSet::default(),
                    kind: NodeKind::FuncCall { callee: callee_emu_group.scc_root },
                    inputs: [current_state].into_iter().collect(),
                    child_regions: [].into_iter().collect(),
                    outputs: [NodeOutputDecl { attrs: AttrSet::default(), ty: state_ty }]
                        .into_iter()
            ...
            func.regions[state_machine_loop_body].children.insert_last(call_node, func.nodes);
            Value::NodeOutput { node: call_node, output_idx: 0 }
        };
        ...
        let state_machine_loop_repeat_cond = {
            ...
            next_state_switch_def.outputs.push(NodeOutputDecl {
                attrs: AttrSet::default(),
                ty: cx.intern(scalar::Type::Bool),
            });
            Value::NodeOutput { node: next_state_switch, output_idx }
        };
...
impl Visitor<'_> for EmuStateReserver<'_> {
    ...
    fn visit_node_def(&mut self, func_at_node: FuncAt<'_, Node>) {
        ...
        let (loop_body, needs_merge_state) = match node_def.kind {
            ...
            NodeKind::Loop { .. } => (Some(node_def.child_regions[0]), false),
            _ => (None, false),
        };
...
impl<'a> EmuGlobalStack<'a> {
    ...
    fn pusher_popper<const CAN_PUSH: bool>(
        ...
        let stack_top_initial = func.nodes.define(
            &self.cx,
            DataInstDef {
                attrs: AttrSet::default(),
                ...
                child_regions: [].into_iter().collect(),
                outputs: [NodeOutputDecl { attrs: Default::default(), ty: self.type_of_stack_top }]
                    .into_iter()
                    .collect(),
            }
            .into(),
        ...
        let stack_ptr = func.nodes.define(
            &self.cx,
            DataInstDef {
                attrs: AttrSet::default(),
                ...
                inputs: [Value::Const(self.ptr_to_stack_array_global), Value::NodeOutput {
                    node: stack_top_initial,
                    output_idx: 0,
                }]
                .into_iter()
                .collect(),
                child_regions: [].into_iter().collect(),
                outputs: [NodeOutputDecl { attrs: Default::default(), ty: self.type_of_stack_ptr }]
                    .into_iter()
                    .collect(),
            }
            .into(),
    ...
    fn collect_cont_closure(
        ...
        let first_func_local_var = func
            ...
            .next()
            .filter(|func_at_inst| {
                matches!(func_at_inst.def().kind, DataInstKind::QPtr(qptr::QPtrOp::FuncLocalVar(_)))
            })
        ...
        let mut collector = EmuContClosureCollector {
            ...
            defined_node_outputs: EntityOrientedDenseMap::new(),
        };
        ...
        for i in 0..collector.closure.input_count {
            ...
            let v = origin.either(
                |(region, _)| Value::RegionInput { region, input_idx: i.try_into().unwrap() },
                |(node, _)| Value::NodeOutput { node, output_idx: i.try_into().unwrap() },
            );
            match collector.transform_value_use_in_func(func.reborrow().at(v)) {
                Transformed::Unchanged => unreachable!(),
                Transformed::Changed(new) => {
                    assert!(
                        new == Value::NodeOutput {
                            node: collector.pops_of_inputs_and_captures[i],
                            output_idx: 0
                        }
                    );
                }
        ...
        if let Some(popper) = collector.popper.take() {
            let pops_nodes = popper.finish(func.reborrow());
            cont_body_def.children.prepend(pops_nodes, func.nodes);
        }
    ...
    fn push(&mut self, mut func: FuncAtMut<'_, ()>, v: Value) {
        ...
        let inst = func.nodes.define(
            ...
            DataInstDef {
                ...
                inputs: [Value::NodeOutput { node: self.stack_ptr, output_idx: 0 }, v]
                    .into_iter()
                    .collect(),
                child_regions: [].into_iter().collect(),
                outputs: [].into_iter().collect(),
    ...
    fn pop(&mut self, func: FuncAtMut<'_, ()>, ty: Type) -> Value {
        ...
        let inst = func.nodes.define(
            ...
            DataInstDef {
                ...
                inputs: [Value::NodeOutput { node: self.stack_ptr, output_idx: 0 }]
                    .into_iter()
                    .collect(),
                child_regions: [].into_iter().collect(),
                outputs: [NodeOutputDecl { attrs: Default::default(), ty }].into_iter().collect(),
            }
            .into(),
        );
        self.push_pop_insts.insert_last(inst, func.nodes);
        ...
        Value::NodeOutput { node: inst, output_idx: 0 }
    }
...
impl<const CAN_PUSH: bool> EmuStackPusherPopper<'_, '_, CAN_PUSH> {
    ...
    fn finish_for_state(
        ...
        stack_overflow_checked_insts
            ...
            .insert_first(self.stack_ptr, func.nodes);
        pre_check_insts.insert_first(self.stack_top_initial, func.nodes);
        ...
        let mut mk_stack_top_plus = |offset_in_stack_units: i32| {
            let inst =
                func.nodes.define(
                    ...
                    DataInstDef {
                        ...
                        inputs: [
                            Value::NodeOutput { node: self.stack_top_initial, output_idx: 0 },
            ...
            if stack_overflow_checked_insts.is_some() {
                ...
                pre_check_insts.insert_before(inst, self.stack_ptr, func.nodes);
            }
        ...
        if let Some(stack_top_plus_min_neg_offset) = stack_top_plus_min_neg_offset {
            ...
            *stack_ptr_idx_input =
                Value::NodeOutput { node: stack_top_plus_min_neg_offset, output_idx: 0 };
            ...
            while let Some(func_at_inst) = func_at_insts.next() {
                ...
                assert!(
                    inst_def.inputs[0] == Value::NodeOutput { node: self.stack_ptr, output_idx: 0 }
                );
            }
        }
        ...
        let stack_top_store = func.nodes.define(
            ...
            DataInstDef {
                ...
                inputs: [
                    ...
                    Value::NodeOutput { node: stack_top_plus_final_offset, output_idx: 0 },
                ]
                .into_iter()
                .collect(),
                child_regions: [].into_iter().collect(),
        ...
        let would_overflow_stack = func.nodes.define(
            cx,
            DataInstDef {
                attrs: AttrSet::default(),
                ...
                inputs: [
                    Value::NodeOutput {
                        node: stack_top_plus_min_neg_offset.unwrap(),
                        output_idx: 0,
                    },
                    Value::NodeOutput { node: self.stack_top_initial, output_idx: 0 },
                ]
        ...
        all_nodes.insert_last(would_overflow_stack, func.nodes);

        let cases =
            [(None, EmuStateIdx::STACK_OVERFLOW.to_value(cx)), (Some(checked_insts), next_state)]
                .into_iter()
                .map(|(insts, output)| {
                    func.regions.define(cx, RegionDef {
        ...
        let check_node = func.nodes.define(
            ...
            NodeDef {
                ...
                inputs: [Value::NodeOutput { node: would_overflow_stack, output_idx: 0 }]
                    .into_iter()
                    .collect(),
                child_regions: cases,
                outputs: [NodeOutputDecl {
        ...
        (all_nodes, Value::NodeOutput { node: check_node, output_idx: 0 })
    }
}
...
struct EmuContClosureCollector<'a, 'b> {
    ...
    defined_region_inputs: EntityOrientedDenseMap<Region, ()>,
    defined_node_outputs: EntityOrientedDenseMap<Node, ()>,
}
...
impl Transformer for EmuContClosureCollector<'_, '_> {
    fn transform_value_use_in_func(
        ...
        let already_valid = match v {
            Value::Const(_) => true,
            Value::RegionInput { region, .. } => self.defined_region_inputs.get(region).is_some(),
            Value::NodeOutput { node, .. } => self.defined_node_outputs.get(node).is_some(),
        };
        if already_valid {
        ...
        let cont_input_idx = match (origin_region_or_node, v) {
            (Either::Left(r1), Value::RegionInput { region: r2, input_idx }) if r1 == r2 => {
                Some(input_idx)
            }
            (Either::Right(n1), Value::NodeOutput { node: n2, output_idx }) if n1 == n2 => {
                Some(output_idx)
            }
            _ => None,
        };
        ...
        if let Some(&inst) = self.pops_of_inputs_and_captures.get(pop_idx) {
            ...
            return Transformed::Changed(Value::NodeOutput { node: inst, output_idx: 0 });
        }
    ...
    fn in_place_transform_region_def(&mut self, mut func_at_region: FuncAtMut<'_, Region>) {
        self.defined_region_inputs.insert(func_at_region.position, ());
    ...
    fn in_place_transform_node_def(&mut self, mut func_at_node: FuncAtMut<'_, Node>) {
        ...
        self.defined_node_outputs.insert(func_at_node.position, ());
    }
}
...
impl EmuStateSplitter<'_> {
    fn maybe_split_states_in_region(
        ...
        let mut cont_body = {
            ...
            let (children, next_state_after) = pusher
        ...
        while let Some(node) = func.regions[region].children.remove_last(func.nodes) {
            if let Some(&node_states) = self.states.for_node.get(node) {
                ...
                self.state_switch_cases
                    .insert(node_states.merge, func.regions.define(cx, merge_cont_body_def));

                cont_body = self.split_states_in_node(func.reborrow().at(node), merge_cont);
            } else {
    ...
    fn split_states_in_node(
        ...
        let next_state_after = match &mut node_def.kind {
            ...
            NodeKind::Select(_) => {
                ...
                func.reborrow().at(node).def().outputs =
                    [NodeOutputDecl { attrs: AttrSet::default(), ty: state_ty }]
                        .into_iter()
                        .collect();
