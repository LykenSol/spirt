...
use crate::{
    ...
    Node, NodeDef, NodeKind, Region, RegionDef, SelectionKind, Type, TypeKind, Value, Var, VarDecl,
    VarKind, scalar,
};
...
impl Visitor<'_> for CallGraphCollector<'_> {
    ...
    fn visit_region_def(&mut self, func_at_region: FuncAt<'_, Region>) {
        ...
        for &input in inputs {
            self.visit_var_decl(func_at_region.at(input));
        }
        for func_at_node in func_at_region.at(*children) {
            ...
            if let (Ok(caller), DataInstKind::FuncCall(callee)) = (self.caller, kind) {
                ...
                for &output in outputs {
                    self.visit_var_decl(func_at_region.at(output));
                }
            } else {
...
impl<'a> CallStackEmulator<'a> {
    ...
    fn transform_all_funcs_in_emu_group(
        ...
        for (func_idx, (&func, func_states)) in per_func_states.iter().enumerate() {
            ...
            let current_state = {
                let input_var = func.vars.define(cx, VarDecl {
                ...
                let next_state_after =
                    match per_func_states.get_index(func_idx + 1).map(|(&f, _)| f) {
                        Some(next_func_in_group) => {
                            ...
                            let call_output_var = func.vars.define(cx, VarDecl {
            ...
            // FIXME(eddyb) automate this (insertion cursor?).
            let state_switch_output_var = func.vars.define(cx, VarDecl {
                attrs: Default::default(),
                ty: state_ty,
    ...
    fn transform_inter_emu_group_func_call(
        ...
        let state_machine_loop_repeat_cond = {
            ...
            // FIXME(eddyb) automate this (insertion cursor?).
            let state_machine_loop_repeat_cond = func.vars.define(cx, VarDecl {
                attrs: Default::default(),
...
impl<'a> EmuGlobalStack<'a> {
    ...
    fn pusher_popper<const CAN_PUSH: bool>(
        ...
        // FIXME(eddyb) automate this (insertion cursor?).
        let stack_top_initial = func.vars.define(&self.cx, VarDecl {
            attrs: Default::default(),
        ...
        // FIXME(eddyb) automate this (insertion cursor?).
        let stack_ptr = func.vars.define(&self.cx, VarDecl {
            attrs: Default::default(),
...
impl<const CAN_PUSH: bool> EmuStackPusherPopper<'_, '_, CAN_PUSH> {
    ...
    fn finish_for_state(
        ...
        // FIXME(eddyb) automate this (insertion cursor?).
        let would_overflow_stack = func.vars.define(cx, VarDecl {
            attrs: Default::default(),
            ty: cx.intern(scalar::Type::Bool),
        ...
        // FIXME(eddyb) automate this (insertion cursor?).
        let check_output_var = func.vars.define(cx, VarDecl {
            attrs: Default::default(),
...
struct EmuContClosureCollector<'a, 'b> {
    ...
    /// Outputs of `qptr.load`s (each added to `pops_block`) popping all inputs
...
impl EmuStateSplitter<'_> {
    fn maybe_split_states_in_region(
        ...
        let mut cont_body = {
            ...
            let values_in_pop_order = outputs
                .into_iter()
                .chain(next_cont_after_region.captures.iter().map(|&v| Value::Var(v)));
    ...
    fn split_states_in_node(
        ...
        let next_state_after = match &mut node_def.kind {
            ...
            DataInstKind::FuncCall(callee) => {
                ...
                let values_in_push_order =
                    (next_cont_after_node.captures.iter().map(|&v| Value::Var(v)).rev())
