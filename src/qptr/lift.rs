...
impl<'a> LiftToSpvPtrs<'a> {
    ...
    fn try_lift_global_var_init(
        ...
        let is_aggregate = |candidate_layout: &MemTypeLayout| {
            matches!(&self.cx[candidate_layout.original_type].kind, TypeKind::SpvInst {
    ...
    fn spv_pointee_type_and_addr_space_for_global_var(
        ...
        let pointee_type = self.pointee_type_for_shape_and_usage(shape, qptr_usage)?;
    ...
    fn pointee_type_for_shape_and_usage(
        &self,
        ...
        match (shape, usage) {
            (
                ...
                QPtrUsage::Handles(handle_usage),
            ) => {
                let handle_type = match (handle, handle_usage) {
                    (shapes::Handle::Opaque(ty), &shapes::Handle::Opaque(usage_ty)) => {
                        if usage_ty != ty {
                            return Err(LiftError(Diag::bug([
                    ...
                    (shapes::Handle::Buffer(_, buf), shapes::Handle::Buffer(_, data_usage)) => {
                    ...
                    _ => {
                        return Err(LiftError(Diag::bug([
                            "mismatched `qptr.usage` and `shape`".into()
                        ])));
                    }
                };
            ...
            _ => Err(LiftError(Diag::bug(["mismatched `qptr.usage` and `shape`".into()]))),
        }
    }
...
impl LiftToSpvPtrInstsInFunc<'_> {
    ...
    fn type_of_val_as_spv_ptr_with_layout(
        ...
        if let Value::Var(v) = v {
            if let Some(ptr_noop) = self.deferred_ptr_noops.get(&v) {
                return Ok((
    ...
    fn try_lift_data_inst_def(
        ...
        let replacement_data_inst_def = match &data_inst_def.kind {
            ...
            &DataInstKind::QPtr(QPtrOp::FuncLocalVar(mem_layout)) => {
                let output_qptr_usage = self
                    .lifter
                    .require_qptr_usage_attr(func.at(data_inst_def.outputs[0]).decl().attrs)?;

                // HACK(eddyb) reusing the same functionality meant for globals.
                let pointee_type = self.lifter.pointee_type_for_shape_and_usage(
            ...
            DataInstKind::QPtr(op @ (QPtrOp::Load { offset } | QPtrOp::Store { offset })) => {
                ...
                let (spv_opcode, stored_value) = match op {
                    QPtrOp::Load { .. } => (wk.OpLoad, None),
                    QPtrOp::Store { .. } => (wk.OpStore, Some(data_inst_def.inputs[1])),
                    _ => unreachable!(),
                };
                ...
                let valid_bitwrangling_access =
                    (pointee_type != access_type).then_some(()).and_then(|()| {
                        ...
                        let access_scalar_width =
                            scalar::IntWidth::try_from_bits(access_scalar_type.bit_width())?;
                ...
                let load_pointee_inst = insert_aux_data_inst(self, func.reborrow(), DataInstDef {
                    attrs: Default::default(),
                    kind: DataInstKind::SpvInst(wk.OpLoad.into(), spv::InstLowering::default()),
                    inputs: [adjusted_ptr].into_iter().collect(),
                    child_regions: [].into_iter().collect(),
                    outputs: [].into_iter().collect(),
                });
                let loaded_pointee = func.vars.define(cx, VarDecl {
                    attrs: Default::default(),
                    ty: pointee_type,
                ...
                let new_kind_and_inputs = if let Some(stored_value) = stored_value {
                    ...
                    let mask_loaded_pointee_inst =
                        insert_aux_data_inst(self, func.reborrow(), DataInstDef {
                            attrs: Default::default(),
                    ...
                    let masked_loaded_pointee = func.vars.define(cx, VarDecl {
                        attrs: Default::default(),
                        ty: pointee_type,
                    ...
                    let zext_stored_value = func.vars.define(cx, VarDecl {
                        attrs: Default::default(),
                        ty: pointee_type,
                    ...
                    let shifted_left_stored_value = if shl_amount == 0 {
                        ...
                        let shifted_left_stored_value = func.vars.define(cx, VarDecl {
                            attrs: Default::default(),
                            ty: pointee_type,
                    ...
                    let merged_value = func.vars.define(cx, VarDecl {
                        attrs: Default::default(),
                        ty: pointee_type,
                    ...
                    let shifted_right_pointee = if shr_amount == 0 {
                        ...
                        let shifted_pointee = func.vars.define(cx, VarDecl {
                            attrs: Default::default(),
                            ty: pointee_type,
                    ...
                    let (trunc_scalar_ty, trunc_op) = match bw.access_scalar_type {
                        ...
                        scalar::Type::SInt(_) => (
                            ...
                            scalar::IntUnOp::TruncOrSignExtend,
                        ),
                        scalar::Type::UInt(_) | scalar::Type::Float(_) => (
                            scalar::Type::UInt(bw.access_scalar_width),
                            scalar::IntUnOp::TruncOrZeroExtend,
                        ),
                    };
                    let (trunc_kind, trunc_inputs) = (
                        scalar::Op::IntUnary(trunc_op).into(),
                        [shifted_right_pointee].into_iter().collect(),
                    );
                    ...
                    if trunc_ty == access_type {
                        ...
                        let trunc_inst = insert_aux_data_inst(self, func.reborrow(), DataInstDef {
                            attrs: Default::default(),
                            kind: trunc_kind,
                            inputs: trunc_inputs,
                            child_regions: [].into_iter().collect(),
                        ...
                        let trunc_output = func.vars.define(cx, VarDecl {
                            attrs: Default::default(),
                            ty: trunc_ty,
                        ...
                        (
                            ...
                            [Value::Var(trunc_output)].into_iter().collect(),
                        )
                    }
                };
    ...
    fn adjust_pointer_for_offset_and_usage(
        ...
        let mk_access_chain = |this: &mut Self,
            ...
            if access_chain_inputs.len() > 1 {
                let node = insert_aux_data_inst(this, func.reborrow(), DataInstDef {
                    attrs: Default::default(),
                ...
                let output_var = func.vars.define(cx, VarDecl {
                    attrs: Default::default(),
        ...
        loop {
            ...
            // (tho could it easily do that w/o the pre-lift usage analysis?).
            if !target_fits_in_pointee && access_chain_inputs.len() == 1 {
                ...
                let mut maybe_recompose_dyn_indexing = || {
                    ...
                    let combined_index_output_var = func.vars.define(cx, VarDecl {
                        attrs: Default::default(),
                        ty: array_index_ty,
            ...
            let is_compatible = has_compatible_offset && {
                match target_usage.kind {
                    QPtrMemUsageKind::Unused
                    | QPtrMemUsageKind::OffsetBase(_)
                    | QPtrMemUsageKind::DynOffsetBase { .. } => true,
            ...
            match (&pointee_layout.components, offset) {
                (
                    ...
                    let index = if index_multiplier == 1 {
                        ...
                        // take advantage of it in `qptr::analyze`.
                        return Err(LiftError(Diag::bug([format!(
                    ...
                    // to `adjust_pointer_for_offset_and_usage`.
                    // FIXME(eddyb) consider tracking representations of `qptr`s
                    // that deviate from "`Value` of SPIR-V logical pointer type".
