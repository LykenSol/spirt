...
use crate::qptr::{QPtrAttr, QPtrOp, const_data, shapes};
        ...
        let result = aggregate_layout.deeply_flatten_if(
            ...
            &mut |leaf_offset, leaf| {
                ...
                let err_to_diag = |err| {
                    ...
                    LayoutError(Diag::bug([
                        format!("initializer leaf at offset {leaf_offset}, with value `").into(),
                        leaf_value.into(),
                        format!("`, overlaps with leaf at offsets {offsets:?} (invalid layout?)")
                            .into(),
                    ]))
                };

                // HACK(eddyb) helper shared by `Scalar` and `Vector`.
                let mut total_written_scalar_size = 0;
                let mut write_next_scalar = |leaf_scalar: scalar::Const| {
                    let byte_len = match leaf_scalar.ty() {
                        scalar::Type::Bool => self.layout_cache.config.abstract_bool_size_align.0,
                        scalar::Type::SInt(_) | scalar::Type::UInt(_) | scalar::Type::Float(_) => {
                            let bit_width = leaf_scalar.ty().bit_width();
                            assert_eq!(bit_width % 8, 0);
                            bit_width / 8
                        }
                    };
                    let mut bytes = leaf_scalar.bits().to_le_bytes();
                    ...
                    data.write_bytes(leaf_offset, bytes).map_err(err_to_diag)?;
                    total_written_scalar_size += byte_len;
                    Ok(())
                };

                match &leaf_value_def.kind {
                    ...
                    ConstKind::Undef if leaf_value_def.attrs == AttrSet::default() => {}

                    &ConstKind::Scalar(leaf_scalar) => {
                        write_next_scalar(leaf_scalar)?;
                        assert_eq!(total_written_scalar_size, leaf_size.get());
                    }
                    ...
                    _ => {
                        // FIXME(eddyb) avoid out-of-bounds panics with malformed layouts
                        // (and/or guarantee certain invariants in layouts that didn't error).
                        data.write_symbolic(leaf_offset, leaf_size, leaf_value)
                            .map_err(err_to_diag)?;
                    }
                }
...
impl Transformer for EraseSpvPtrs<'_> {
    ...
    fn transform_const_use(&mut self, ct: Const) -> Transformed<Const> {
        ...
        if let ConstKind::Undef = ct_def.kind {
            if !ct_def.attrs.diags(cx).is_empty() || !cx[ct_def.ty].attrs.diags(cx).is_empty() {
    ...
    fn try_lower_data_inst_def(
            ...
            loop {
                ...
                let Some(new_base_and_offset) = (|| {
                    if let Value::Var(ptr) = ptr {
                        if let VarKind::NodeOutput { node: ptr_inst, output_idx: 0 } =
                            func.at(ptr).decl().kind()
                        {
                            let ptr_inst_def = func.at(ptr_inst).def();
                            if let DataInstKind::QPtr(QPtrOp::Offset(ptr_offset)) =
                                ptr_inst_def.kind
                            {
                                return Some((
                                    ptr_inst_def.inputs[0],
                                    ptr_offset.checked_add(offset)?,
                                ));
                            }
                        }
                    }
        ...
        .contains(&spv_inst.opcode)
        {
            ...
            for step in steps {
                ...
                let step_output_var = func.vars.define(cx, VarDecl {
                    attrs: Default::default(),
                    ty: self.lowerer.qptr_type(),
            ...
            if let Value::Var(size) = size {
                ...
                let index = func.vars.define(cx, VarDecl {
                    attrs: Default::default(),
                    ty: count_ty,
                ...
                let index_plus_one = func.vars.define(cx, VarDecl {
                    attrs: Default::default(),
                    ty: count_ty,
                ...
                let any_left = func.vars.define(cx, VarDecl {
                    attrs: Default::default(),
                ...
                let if_any_left_node = func.nodes.define(
                    ...
                    NodeDef {
                        ...
                        // semantic, and understand the semantic ones.
                        attrs,
                        kind: NodeKind::Select(crate::SelectionKind::BoolCond),
                ...
                let copy_inst = func.nodes.define(
                    ...
                    DataInstDef {
                        ...
                        // semantic, and understand the semantic ones.
                        attrs,
                        kind: QPtrOp::Copy { size: stride }.into(),
            ...
            for (leaf_offset, leaf_type) in leaf_offsets_and_types {
                ...
                let leaf_load_output_var = func.vars.define(cx, VarDecl {
                    attrs: Default::default(),
                    ty: leaf_type,
