//! [`QPtr`](crate::TypeKind::QPtr) usage analysis (for lifting to typed memory).
//
...
impl UsageMerger<'_> {
    ...
    fn merge_mem_at(
        ...
        // the merged usage to be outside the bounds of *both* `a` and `offset => b`.
        ...
        let [a, b] = if b_offset_in_a == 0 {
            ...
            sorted.sort_by_key(|usage| {
                ...
                let type_strictness = match usage.kind {
                    QPtrMemUsageKind::Unused | QPtrMemUsageKind::OffsetBase(_) => {
                        TypeStrictness::Any
                    }
            ...
            // `a` too small to fit `offset => b` as a new `OffsetBase`.
                ...
                [
                    QPtrMemUsage {
                        ...
                        kind: QPtrMemUsageKind::OffsetBase(Rc::new(
                            Some((0, a))
                                .filter(|(_, a)| !matches!(a.kind, QPtrMemUsageKind::Unused))
                                .into_iter()
                                .collect(),
                        )),
                    },
                        ...
                        self.layout_cache
                            ...
                            .and_then(|layout| match layout {
                                ...
                                TypeLayout::Concrete(concrete) => {
                                    ...
                                    let offset = u32::try_from(*offset).ok().ok_or_else(|| {
                                        AnalysisError(Diag::bug([format!(
                                            "{op_name} {{ offset: {offset} }}: negative offset").into()
                                        ]))
                                    })?;
