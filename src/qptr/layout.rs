...
impl<T: Ord> Extent<T> {
    pub(super) fn includes(&self, other: &Self) -> bool {
...
impl Extent<u32> {
    pub(super) fn checked_add(self, delta: u32) -> Option<Self> {
        Some(Self {
    ...
    pub(super) fn saturating_add(self, delta: u32) -> Self {
        Self {
    ...
    fn layout_of_uncached(&self, ty: Type) -> Result<TypeLayout, LayoutError> {
        ...
        let (spv_inst, type_and_const_inputs) = match &ty_def.kind {
            ...
            TypeKind::Vector(ty) => {
                ...
                return array(cx.intern(ty.elem), ArrayParams {
                    fixed_len: Some(len),
                    known_stride: None,
