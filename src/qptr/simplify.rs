...
use crate::{
    AttrSet, Const, ConstDef, ConstKind, Context, ControlNodeOutputDecl, ControlRegion,
    ControlRegionDef, ControlRegionInputDecl, DataInst, DataInstDef, DataInstForm, DataInstKind,
    Func, FuncDefBody, FxIndexMap, FxIndexSet, GlobalVar, Type, TypeKind, vector,
};
                    ...
                    let first_input_qptr_with_offset_and_access_type = match *op {
                        QPtrOp::FuncLocalVar(layout) => {
                            ...
                            if data_inst_def.inputs.is_empty() {
                                self.vars.insert(func_at_inst.position, LocalVarPartitions {
...
impl PropagateLocalVarContents<'_> {
    ...
    fn propagate_through_data_inst(
        ...
        match *kind {
            ...
            DataInstKind::QPtr(QPtrOp::Store { offset }) => {
                ...
                let access_type = func_at_inst.reborrow().freeze().at(stored_value).type_of(cx);
                ...
                if let Some(access) = self.lookup_var_for_access(dst_ptr, offset, access_type) {
                    ...
                    let new_var_value = match access.vector_elem_idx {
                        ...
                        None if var_ty != access_type => {
                            let bitcast_data_inst = func_at_inst.reborrow().data_insts.define(
                                cx,
                                DataInstDef {
                                    // FIXME(eddyb) preserve at least debuginfo attrs.
                                    attrs: Default::default(),
                            ...
                            Value::DataInstOutput { inst: bitcast_data_inst, output_idx: 0 }
                        }
    ...
    fn propagate_through_control_node(&mut self, func_at_control_node: FuncAtMut<'_, ControlNode>) {
        ...
        match &mut func.reborrow().at(control_node).def().kind {
            ...
            | ControlNodeKind::ExitInvocation { kind: _, inputs } => {
                self.propagate_into_values(inputs);
