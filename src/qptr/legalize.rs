...
use crate::qptr::{LayoutConfig, QPtrOp};
use crate::transform::{InnerInPlaceTransform as _, Transformer};
use crate::{
    AddrSpace, Attr, AttrSet, Const, ConstDef, ConstKind, Context, DeclDef, Diag,
    EntityOrientedDenseMap, Func, FuncDefBody, FxIndexMap, FxIndexSet, GlobalVar, GlobalVarDecl,
    GlobalVarDefBody, Module, Node, NodeDef, NodeKind, Region, RegionDef, SelectionKind, Type,
    TypeDef, TypeKind, Value, Var, VarDecl, VarKind, cfg, scalar, spv,
};
...
enum DefLaw {
    ...
    /// complicate `qptr::analyze` (requiring it to do global type inference).
    ...
    /// `qptr.store(m, p)` -> `addr = spv.OpConvertPtrToU(p); qptr.store(m, addr)`
...
enum QPtrDefLaw {
    ...
    /// Pointer output of either `QPtrOp::Load` (i.e. reading a pointer from memory),
    ...
    /// - `p = qptr.load(m)` -> `addr = qptr.load(m); p = spv.OpConvertUToPtr(addr)`
...
#[derive(Copy, Clone)]
enum QPtrUseLaw {
    /// Input to either `QPtrOp::Store` (i.e. writing a pointer to memory),
    ...
    /// (at least for this input, e.g. `QPtrOp::Store` may *also* have its value
...
impl<'a> LegalizePtrs<'a> {
    ...
    pub fn legalize_module(
        ...
        // via `Option<&EntityDefs<GlobalVar>>`.
        {
            ...
            for &gv in &all_uses_from_module.global_vars {
                ...
                let addr_space_of_var_or_buffer =
                    if let Some(crate::qptr::shapes::GlobalVarShape::Handles {
                        handle: crate::qptr::shapes::Handle::Buffer(addr_space, _),
                        ..
                    }) = gv_decl.shape
                    {
        ...
        let mut any_func_ever_encodes_or_decodes_escaped_ptrs = false;
            ...
            if let DeclDef::Present(func_def_body) = &mut module.funcs[func].def {
                ...
                base_maps.escaped.bases.extend(escaping_func_locals.iter().map(
                    |(local, &offset_shape)| {
                        ...
                        let NodeKind::QPtr(QPtrOp::FuncLocalVar(layout)) = node_def.kind else {
                            unreachable!();
                        };

                        // TODO(eddyb) support (by injecting a `qptr.store`
                        // just after the original declaration position).
                        assert_eq!(node_def.inputs.len(), 0);

                        let gv = module.global_vars.define(&self.cx, GlobalVarDecl {
                            // FIXME(eddyb) copy at least debuginfo attrs .
                            attrs: Default::default(),
                            type_of_ptr_to: self.qptr_type(),
                            shape: Some(crate::qptr::shapes::GlobalVarShape::UntypedData(layout)),
                    ...
                    for summary in summarized_ptrs.values_mut() {
                        ...
                        for base in single_base_slots.into_iter().flatten() {
                            if let Base::FuncLocal(local) = *base {
                                if let Some(i) = escaping_func_locals.get_index_of(&local) {
                                    *base =
                                        Base::Global(globals_for_escaping_func_locals.keys()[i]);
                                }
                            }
                        }
                    }
                ...
                any_func_ever_encodes_or_decodes_escaped_ptrs |=
                    ever_encodes_or_decodes_escaped_ptrs;

                base_maps.per_func.insert(func, func_base_map);
                per_func_scan_results.insert(func, FuncScanResults {
                    summarized_ptrs,
                    ever_encodes_or_decodes_escaped_ptrs,
                });
            }
        ...
        if any_func_ever_encodes_or_decodes_escaped_ptrs {
    ...
    fn node_qptr_expansions(&self, func_at_node: FuncAt<'_, Node>) -> QPtrExpansions {
        ...
        if let [output_var] = node_def.outputs[..] {
            if let Some(DefLaw::CmpQPtrEqOrNe { .. }) =
    ...
    fn qptr_def_law(&self, func_at_ptr: FuncAt<'_, Var>) -> QPtrDefLaw {
        ...
        match &node_def.kind {
            ...
            NodeKind::QPtr(op) => {
                ...
                match *op {
                    QPtrOp::FuncLocalVar(_) => QPtrDefLaw::Base(Base::FuncLocal(FuncLocal {
                    ...
                    QPtrOp::Load { .. } => QPtrDefLaw::DecodeEscaped { is_exact_cast: false },
...
impl<'a> LegalizePtrsInFunc<'a> {
    fn new(
        ...
        for (ptr, def_law, get_bases_or_unsupported, offset_shape) in ptrs {
            ...
            match def_law {
                QPtrDefLaw::Offset(new_offset) => {
                    ...
                    match (input_canon_ptr.offset, new_offset) {
                        ...
                        (Offset::Zero, Offset::Strided { stride, index }) => {
                            override_index = Some((stride, match index {
            ...
            let mut decl_var = |ty| {
                func_def_body.vars.define(cx, VarDecl {
            ...
            this.canon_ptrs.insert(ptr, CanonPtr {
                def_law,
    ...
    fn apply_node_qptr_expansions(
        ...
        if let Some(input_idx) = pre_encode_escaping_input {
            ...
            let encode_output_var = func.vars.define(cx, VarDecl {
                attrs: Default::default(),
        ...
        if let Some(output_idx) = post_decode_escaped_output {
            let undecoded_var = func.vars.define(cx, VarDecl {
                attrs: Default::default(),
                ty: self.legalizer.qptr_sized_uint_type(),
        ...
        if let Some((input_idx, CanonPtr { base, offset, .. })) = instantiate_by_input {
            ...
            for choice_or_default in base_choices.map(Some).chain([None]) {
                ...
                // e.g. `qptr::{analyze,lift}` pointlessly making up typed nulls.
                if let Some((_, Base::Global(GlobalBase::Undef | GlobalBase::Null))) =
                    choice_or_default
                {
                    let always_accesses_input_ptr = match (&node_def_template.kind, input_idx) {
                        (NodeKind::QPtr(QPtrOp::Load { .. } | QPtrOp::Store { .. }), 0)
                        | (NodeKind::QPtr(QPtrOp::Copy { .. }), 0..=1) => true,
                        _ => false,
                    };
            ...
            match def_law {
                    ...
                    let [base_cmp_result, offset_cmp_result] =
                        [base_cmp, offset_cmp].map(|cmp| match cmp {
                            ...
                            Either::Right([a, b]) => {
                                ...
                                upstream_cursor.insert_node(cx, NodeDef {
                                    // FIXME(eddyb) copy at least debuginfo attrs.
                                    attrs: Default::default(),
                    ...
                    let byte_offset = match canon_def.base {
                        ...
                        BaseChoice::Many { base_index_bitset: _, base_index_selector } => {
                            ...
                            upstream_cursor.insert_node(cx, NodeDef {
                                ...
                                inputs: [encoded, encoded_offset_bit_width].into_iter().collect(),
                                child_regions: [].into_iter().collect(),
                            ...
                            upstream_cursor.insert_node(cx, NodeDef {
                                ...
                                inputs: [Value::Var(base_index_selector), encoded_offset_bit_width]
                                    .into_iter()
                                    .collect(),
                                child_regions: [].into_iter().collect(),
                ...
                DefLaw::EncodeEscapingQPtr => {
                        ...
                        canon_ptr.base.iter_with_selector_value(&self.func_base_map).1
                    {
                        ...
                        use crate::qptr::shapes::{GlobalVarShape, Handle};
                    ...
                    let base_encoded_start = if encoded_base_bit_width == 0 {
                        ...
                        upstream_cursor.insert_node(cx, NodeDef {
                            ...
                            inputs: [
                                base_index_selector,
                                Value::Const(
                                    cx.intern(scalar::Const::from_u32(encoded_offset_bit_width)),
                                ),
                            ]
                            .into_iter()
                            .collect(),
                ...
                DefLaw::QPtr(QPtrDefLaw::Unsupported) => {
                    ...
                    for DynSink { upstream: _, values } in &mut dyn_sinks {
                        if let Ok(canon) = self.try_use_canonical_ptr(values.old[original_idx]) {
                            if let BaseChoice::Many { .. } = canon.base {
                                values.replace_old_with_new(original_idx, [Value::Const(
                                    self.legalizer.undef_qptr_const(),
                                )]);
                            }
                        }
                    }
            ...
            BaseChoice::Single(Base::Global(GlobalBase::BufferData { ptr_to_buffer })) => {
                match canon_ptr.single_base_buffer_data.unwrap() {
                    DefOrUse::Def(buffer_data_output_var) => {
                        cursor.insert_node(&self.legalizer.cx, NodeDef {
    ...
    fn legalize_offset_sum(
        ...
        let lhs_ty = cursor.reborrow().freeze().at(lhs.value).type_of(&self.legalizer.cx);
        let index_ty = match rhs.value {
            IndexValue::Dyn(index_value) => self.legalizer.merge_scaled_index_types(
                lhs.map_value(|_| lhs_ty),
                rhs.map_value(|_| {
                    cursor.reborrow().freeze().at(index_value).type_of(&self.legalizer.cx)
                }),
            ),
...
impl Transformer for LegalizePtrsInFunc<'_> {
    ...
    fn in_place_transform_node_def(&mut self, mut func_at_node: FuncAtMut<'_, Node>) {
        ...
        match node_def.inputs.iter().filter(|&&v| is_multi_base_qptr(v)).count() {
            ...
            1 => {
                self.apply_node_qptr_expansions(func.at(node), QPtrExpansions {
                    pre_encode_escaping_input: None,
                    post_decode_escaped_output: None,
                    instantiate_by_multi_base_qptr_inputs: true,
                });
