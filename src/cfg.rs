...
use crate::{
    ...
    FxIndexMap, FxIndexSet, Node, NodeDef, NodeKind, Region, RegionDef, SelectionKind, Type,
    TypeKind, Value, Var, VarDecl, VarKind, spv,
};
        ...
        let deferred_edges = deferred_targets.into_iter().map(|(target, target_summary)| {
            ...
            let target_inputs = (0..input_count)
                .map(|target_input_idx| {
                    ...
                    let output_var = self.func_def_body.vars.define(self.cx, VarDecl {
                        attrs: AttrSet::default(),
                        ty,
