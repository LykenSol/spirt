...
use crate::{
    ...
    ControlNodeOutputDecl, ControlRegion, ControlRegionDef, DbgSrcLoc, EntityOrientedDenseMap,
    FuncDefBody, FxIndexMap, FxIndexSet, SelectionKind, Type, Value, scalar, spv,
};
             ...
             cases: &[Result<ClaimedRegion, DeferredEdgeBundleSet>]| {
                ...
                let max_cases_line_col = cases
                    ...
                    .filter_map(|case| {
                        ...
                        // FIXME(eddyb) maybe there should be a `FuncAt<ControlRegion>`
                        // helper for "debug locations from all `Block` `DataInst`s
                        // and non-`Block` `ControlNode`"? (i.e. only flattening `Block`s)
                        this.func_def_body
                            .at(structured_body)
                            .at_children()
                            .into_iter()
                            .flat_map(|func_at_child| {
                                ...
                                if let ControlNodeKind::Block { insts } = child_def.kind {
                                    Either::Left(
                                        func_at_child
                                            .at(insts)
                                            .into_iter()
        ...
        let mut get_or_define_select_node = |this: &mut Self, cases: &[_]| {
            *cached_select_node.get_or_insert_with(|| {
                ...
                if let Some(select_dbg_src_loc) = attrs.dbg_src_loc(this.cx) {
                    if let Some(dbg_src_loc) = extend_dbg_src_loc(this, select_dbg_src_loc, cases) {
    ...
    fn materialize_lazy_cond(&mut self, cond: &LazyCond) -> Value {
        match cond {
            ...
            LazyCond::Merge(merge) => {
                ...
                let ControlNodeDef { attrs: _, kind, outputs: output_decls } =
