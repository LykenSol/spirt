...
use crate::{
    ...
    NodeKind, OrdAssertEq, Region, RegionDef, SelectionKind, Type, TypeDef, TypeKind, TypeOrConst,
    Value, Var, VarDecl, cfg, scalar, spv, vector,
};
            ...
            for func_def_body in func_def_bodies_across_versions {
                ...
                let visit_region = |func_at_region: FuncAt<'_, Region>| {
                    ...
                    for &input_var in inputs {
                        define(Use::Var(input_var), Some(func_def_body.at(input_var).decl().attrs));
                    }
        ...
        match style {
            ...
            UseStyle::Inline => match *self {
                ...
                Self::PlanItem(item) => printer
                    ...
                    .into(),
                Self::RegionLabel(_) | Self::Var(_) => "_".into(),
...
impl Print for FuncAt<'_, Node> {
    ...
    fn print(&self, printer: &Printer<'_>) -> pretty::Fragment {
        ...
        let node_body = match kind {
            ...
            NodeKind::ExitInvocation(cfg::ExitInvocationKind::SpvInst(spv::Inst {
                opcode,
                imms,
            })) => printer.pretty_spv_inst(
                kw_style,
