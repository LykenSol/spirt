...
impl AttrSetDef {
    ...
    // HACK(eddyb) these only exists to avoid changing code working with `AttrSetDef`s.
...
//
// FIXME(eddyb) consider labeling this in some way to indicate (and require)
// that it cannot contain uses of entities (and in terms of interned values,
// it only references `InternedStr` and `Type` - see also `DataInstKind`).
#[derive(Clone, PartialEq, Eq, Hash)]
...
// FIXME(eddyb) consider labeling this in some way to indicate (and require)
// that it cannot contain uses of entities (and in terms of interned values,
// it only references `InternedStr`, and `Type` through `spv::InstLowering`).
#[derive(Clone, PartialEq, Eq, Hash, derive_more::From)]
