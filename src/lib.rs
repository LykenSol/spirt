...
pub enum GlobalVarInit {
    ...
    /// Explicitly laid out constant data, using [`qptr::const_data::ConstData`]
    /// to efficiently mix concrete bytes with symbolic ([`Const`]) values.
    Data(qptr::const_data::ConstData<Const>),
}
...
pub enum NodeKind {
    ...
    ExitInvocation(cfg::ExitInvocationKind),
...
// also mention `GlobalVar`/`qptr::QPtrOp::FuncLocalVar`.
pub use context::Var;
